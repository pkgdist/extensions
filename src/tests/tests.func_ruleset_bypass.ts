...
Deno.test({
  ...
  async fn() {
    ...
    if (!token) {
      throw new Error('No GitHub token found. Please set the PROD_TOKEN environment variable.')
    }
    const repoData = {
      token,
    ...
    if (bypassActors && bypassActors.length > 0) {
      ...
      for (const actor of bypassActors) {
        ...
        assert("bypass_mode" in actor, "bypass_mode should exist in bypass actor")
      }
    } else {
